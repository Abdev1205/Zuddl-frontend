...
const CardEditModal = ({ visible, onClose = () => { }, callback = () => { }, stageId, editId }) => {
  ...
  const [cardVisiblity, setCardVisiblity] = useState()
  ...
  const fetchDataForUpdating = async () => {
    ...
    setCardId(response.data.cardData._id)
    setCardTitle(response.data.cardData.title)
    setCardDesc(response.data.cardData.desc)
    setCardPriority(response.data.cardData.priority)
    setCardVisiblity(response.data.cardData.visibility)
  }

  const updateCardsByData = async (e) => {
    e.preventDefault()
    ...
    <>
      ...
      >
        <div className='  flex flex-col w-fit  bg-white px-[2.5rem] py-[2.5rem] rounded-md shadow-2xl ' >
          <form onSubmit={(e) => updateCardsByData(e)} className=' flex flex-col gap-[1rem] relative  ' >
            ...
            <button type='submit' className={` ${updatingCard ? "animate-pulse" : ""} flex gap-[.8rem] justify-center items-center bg-primary py-[.5rem] text-white rounded-md mt-[2rem] `} > {updatingCard ? "Updating Cards" : "Update card"}
              <div className={` ${updatingCard ? "" : "hidden"} w-[1rem] h-[1rem] border-t-2  border-white rounded-[50%] animate-spin `} />
            </button>
          </form>
